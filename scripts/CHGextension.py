...
class CHGDenoiser(CFGDenoiser):
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, s_min_uncond, image_cond):
        ...
        if tensor.shape[1] == uncond.shape[1] or skip_uncond:
            ...
            if shared.opts.batch_cond_uncond:
                x_out = self.Chara_iteration(None, x_in, sigma_in, uncond, cond_scale, conds_list, cond=make_condition_dict(cond_in, image_cond_in))
                print("b0")
            else:
                print("b1")
    ...
    def Chara_iteration(self, dxs, x_in, sigma_in, uncond, cond_scale, conds_list, cond=None ):
        ...
        elif isinstance(self.inner_model, CompVisTimestepsDenoiser) or isinstance(self.inner_model, CompVisTimestepsVDenoiser):
            t_in = sigma_in
        ...
        for iteration in range(n_iterations):
            ...
            if g_1 is None:
                ...
                g_1 = g_1/torch.maximum(g_1_norm, torch.ones_like(g_1_norm)*1e-4)# + self.noise_level*noise/torch.sum( noise**2, dim = (-1,-2) , keepdim=True )
            ...
            dxs_Acc,g_Acc, reg_dxs_Acc, reg_g_Acc = AndersonAccR(dxs, g, reg_Acc, reg_target, pre_condition=None, m = self.aa_dim+1)
            #print(Accout)
            #
...
class ExtensionTemplateScript(scripts.Script):
        ...
        def process_batch(self, p, reg_ini, reg_range, ite, noise_base, chara_decay, res, lr, reg_size, reg_w, aa_dim, checkbox, **kwargs):
                def modified_sample(sample):
                        def wrapper(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
                                ...
                                if checkbox:
                                        ...
                                        try:
                                                result = sample(conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts)
                                                paranames = ["Characteristic Guidance","Regularization Strength","Regularization Range Over Time",
                                                             "Max Num. Characteristic Iteration", "Num. Basis for Correction", "Reuse Correction of Previous Iteration",
                                                             "Log 10 Tolerance for Iteration Convergence", "Iteration Step Size", "Regularization Annealing Speed",
                                                             "Regularization Annealing Strength", "AA Iteration Memory Size"]
                                                paravalues = [checkbox, reg_ini, reg_range, ite, noise_base, chara_decay, res, lr, reg_size, reg_w, aa_dim]
